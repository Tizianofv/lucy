...
def test_cuantos_falsos_positivos_hay_hoy_sobre_los_archivos_reales():
    ...
    #
    # 9-sep-2026: 68 → 71 en disco, sumando DOS trabajos que se juntaron el mismo
    # día y que por separado midieron 69 y 70. Ninguno de los dos números servía
    # ya juntos, y hubo que rehacer la cuenta a mano al fusionar — está anotado
    # en la cola de la sala como el coste concreto de un contador tecleado.
    #
    # Lo que aporta cada uno:
    #   · `tests/test_tarea_a_mano.py`   (alta de tareas a mano en el panel) → EXENTO por rol
    #   · `tests/test_panel_descrito.py` (las pantallas que Lucy sabe que hay)  → EXENTO por rol
    #   · `web/menu.py` —la lista de pantallas leída del menú real, para que la
    #     descripción que lee Lucy no se separe de lo que el panel tiene— → VIGILADO,
    #     que es lo que tiene que pasar con código nuevo fuera de `testpaths`.
    #
    ...
    #
    # 22-sep-2026, encargo 5 (Los proyectos se ven): 78 → 79 en disco y
    # 38 → 39 EXENTOS por `tests/test_proyectos_panel.py` (página de
    # proyectos, botón «convertir en proyecto», huella de
    # `buscar_o_crear_proyecto`, cambiar el área desde el panel). Es un
